...
{ NEWMEM_UNLOCK=$(whiptail --inputbox "Setting to 1024 is the Maximum. It may be prudent to say 1024" 20 60 "16" 3>&1 1>&2 2>&3)
if [ $? -eq 0 ]; then
sudo sed -i '/gpu_mem/ d' /boot/config.txt
sudo echo "gpu_mem=$NEWMEM_SPLIT" | sudo tee --append /boot/config.txt
fi }
sudo sed -i '/total_mem/ d' /boot/config.txt
sudo echo "gpu_mem=$NEWMEM_UNLOCK" | sudo tee --append /boot/config.txt
else
sudo sed -i '/total_mem/ d' /boot/config.txt
fi }
